...
| `--cache-dir`           | кеш-директория ОС    | Директория для кеш-файлов                    |
...
Тесты запускаются с `-race` флагом и включают бенчмарки. Релизные бинарники собираются
через [garble](https://github.com/burrowers/garble) (`garble -tiny build -ldflags="-s -w"`),
который удаляет все имена символов, отладочную информацию и строковые литералы. Это даёт
значительно меньший размер бинарника и помогает избежать ложных срабатываний антивирусов
(многие AV-движки помечают стандартные Go-бинарники из-за большого рантайма и встроенных
метаданных).
